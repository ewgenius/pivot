...
TimeHelper.defaultGranularities = TimeHelper.checkers.map((c) => { return granularityFromJS(c.returnValue); }).concat(TimeHelper.minGranularity).reverse();
TimeHelper.coarseGranularities = TimeHelper.coarseCheckers.map((c) => { return granularityFromJS(c.returnValue); }).concat(TimeHelper.minGranularity).reverse();
...
NumberHelper.defaultGranularities = NumberHelper.checkers.map((c) => { return granularityFromJS(c.returnValue); }).reverse();
...
function getLineChartTicks(range, timezone) {
    if (range instanceof plywood_1.TimeRange) {
        ...
        const tickDuration = getBestBucketUnitForRange(range, true);
